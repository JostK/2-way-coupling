...
!~      stop
									!JK write new velocities/Phi for debugging (2D)
									IF (.TRUE.)	THEN

									Open(fileop(10),file='Ecrash.chk',status='unknown')

									DO p=1, SIZE(FineGrid%x) 

												!write(fileop(15),*)  FineGrid%x(p,1), Wavefield%Px(p,1), Wavefield%W(p,1)
												write(fileop(10),*)  FineGrid%x(p,1), Wavefield%E(p,1), Wavefield%Ex(p,1), Wavefield%Exx(p,1)
												!WRITE(*,*) EOF(p)
									END DO
