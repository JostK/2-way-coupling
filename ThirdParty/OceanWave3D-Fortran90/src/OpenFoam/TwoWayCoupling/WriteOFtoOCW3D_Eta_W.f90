SUBROUTINE WriteOFtoOCW3D_Eta_W
!
! Writes the free surface elevation and velocities calculated by OpenFOAM to the OCW3D solution
!
! By Jost Kemper
USE GlobalVariables, ONLY: WaveField, OfPoints, EOF, UzOF, alpha, beta, gamma, FineGrid,curvilinearONOFF,& 
...
Write(*,*) 'Writing OpenFOAM solution to OceanWave3D (Eta and W only)'
...
! Get new Phi by timeintegration with newly obtained W and Eta (but old Px and Py)
...
!Update derivates of new Phi and smooth
IF (curvilinearONOFF==0) THEN
	IF (FineGrid%Nx>1) THEN
...
END SUBROUTINE WriteOFtoOCW3D_Eta_W
