...
namespace Foam
{
    defineTypeNameAndDebug(OCWprobes, 0);
}


// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::OCWprobes::findElements(const fvMesh& mesh)
{
    if (debug)
    {
        Info<< "OCWprobes: resetting sample locations" << endl;
    }
    ...
    forAll(*this, probeI)
    {
        ...
        {
            Pout<< "OCWprobes : found point " << location
    ...
    // Check if all OCWprobes have been found.
    forAll(elementList_, probeI)
    {
        ...
        else if (faceI == -1)
        {
            if (Pstream::master())
            {
                WarningIn("OCWprobes::findElements(const fvMesh&)")
                    << "Did not find location " << location
...
Foam::label Foam::OCWprobes::prepare()
{
...
//JK: functuion to change probe locations
void Foam::OCWprobes::UpdateProbes(const List<point>& OfPoints)
{
	this -> clear();
...
void Foam::OCWprobes::end()
{
    // Do nothing - only valid on write
}


void Foam::OCWprobes::timeSet()
{
    // Do nothing - only valid on write
}
...
void Foam::OCWprobes::read(const dictionary& dict)
{
...
void Foam::OCWprobes::updateMesh(const mapPolyMesh& mpm)
{
    if (debug)
    {
...
void Foam::OCWprobes::movePoints(const polyMesh&)
{
    if (debug)
    {
