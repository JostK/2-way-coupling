...
template<class Type>
void Foam::OCWpatchProbes::sampleAndWrite
(
...
)
{
    ...
    if (Pstream::master())
    {
        ...
        OFstream& probeStream = *OCWprobeFilePtrs_[vField.name()];
...
)
{
    ...
    {
        unsigned int w = IOstream::defaultPrecision() + 7;
        OFstream& probeStream = *OCWprobeFilePtrs_[sField.name()];
...
template<class Type>
void Foam::OCWpatchProbes::sampleAndWriteSurfaceFields
(
    const fieldGroup<Type>& fields
)
{
...
template<class Type>
Foam::tmp<Foam::Field<Type> >
Foam::OCWpatchProbes::sample(const word& fieldName) const
{
    return sample
    (
