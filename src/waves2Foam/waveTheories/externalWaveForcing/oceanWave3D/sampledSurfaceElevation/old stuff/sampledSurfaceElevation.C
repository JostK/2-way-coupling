...
(
    //JK: fieldGroup<scalar>& fields
)
{
	//JK: lokal copy of scalarFields_
	fieldGroup<scalar> fields = scalarFields_;
	
    //if (fields.size() && performAction())
    if (performAction() && size() && checkFieldTypes())
    {
...
//JK: write function for 2-way coupling to OceanWave3D 
